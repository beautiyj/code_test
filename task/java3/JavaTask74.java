...
class BoardSVC {
    ...
    public void modifyArticle(Scanner sc) {
        System.out.println("수정할 글의 작성자와 비밀번호 입력 하세요?");
        System.out.print("작성자:");
        String register = sc.next();
        System.out.print("비밀번호:");
        String passwd = sc.next();
        ...
        if (boardList.size() > 0) {
            ...
            if (index == -1) {
                ...
                System.out.println("수정할 글의 제목과 내용을 입력 하세요?");
                System.out.print("제목:");
                String subject = sc.next();
                System.out.print("글내용:");
                String content = sc.next();
                ...
                // 해당 인덱스에서 변경
                boardList.get(index).setSubject(subject);
    ...
    public void modifyPw(Scanner sc) {
        ...
        if (boardList.size() > 0) {
            ...
            if (index == -1) {
                ...
                System.out.println("새 비밀번호를 입력 하세요?");
                System.out.print("새 비밀번호:");
                String newPasswd = sc.next();

                // 해당 인덱스에서 변경. 중복된 항목 중 찾은 마지막 인덱스의 비밀번호를 변경
                boardList.get(index).setPasswd(newPasswd);
    ...
    public class JavaTask74 {
        public static void main(String[] args) {
            ...
            do {
                ...
                System.out.println("6.게시글 비밀번호 변경");
                ...
                switch (menu) {
                    case "1":
                        ...
                        break;
                    case "2":
                        boardSVC.listArticles();
                        break;
                    case "3":
                        boardSVC.removeArticle(sc);
                        break;
                    case "4":
                        boardSVC.modifyArticle(sc);
                        break;
                    case "6":
                        boardSVC.modifyPw(sc);
                        break;
                    case "5":
                        isStop = true;
                }
