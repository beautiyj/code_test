...
// borad 게시판 작성하기 (MySQL 버전) JavaTask103파일 활용

/*
-- MySQL용 테이블 생성 쿼리
CREATE TABLE borad(
    no int AUTO_INCREMENT PRIMARY KEY, -- 오라클의 시퀀스 대신 자동 증가 설정
    writer varchar(20) not null,
    pw VARCHAR(20) not null,
    title VARCHAR(100) not null,       -- 기존 코드의 필드명(title)에 맞춤
    content VARCHAR(1000) not null,
    reg_date DATETIME                  -- TIMESTAMP 또는 DATETIME 사용
);
*/
...
class BoardTask106 {
    // MySQL 드라이버 및 접속 정보로 변경
    private static final String DRIVER = "com.mysql.cj.jdbc.Driver";
    private static final String URL = "jdbc:mysql://localhost:3306/jsptest?serverTimezone=UTC";
    private static final String USER = "jspid";
    private static final String PASSWORD = "jsppass";

    // 게시판 글 입력하기 (INSERT)
    public static class InsertBoard {
        public static void main(String[] args) {
            String sql = "INSERT INTO borad(writer, pw, title, content, reg_date) " +
                    "VALUES (?, ?, ?, ?, now())";
    ...
    public static class UpdateBoard {
        public static void main(String[] args) {
            ...
            String sql = "UPDATE borad SET writer = ?, title = ?, content = ?, reg_date = now() WHERE no = ? AND pw = ?";
...
public class JavaTask106 {
    public static void main(String[] args) {
//         BoardTask106.InsertBoard.main(args);
//        BoardTask106.UpdateBoard.main(args);
        // BoardTask106.DeleteBoard.main(args);
         BoardTask106.SelectBoard.main(args);
    }
}