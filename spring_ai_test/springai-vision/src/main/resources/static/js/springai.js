...
springai.vision.initCanvas = function(imgId, canvasId) {
	...
	// 캐시된 이미지 등의 이유로 스크립트 실행 전에 이미 load 이벤트가 발생해서
	// 위 리스너가 호출되지 못하는 경우를 대비해, 이미 로드가 끝난 상태라면 즉시 캔버스 크기를 맞춤
...
springai.vision.resizeCanvas = function(imgId, canvasId) {
	...
	// 이미지(원본)가 새로 바뀌었을 수 있으므로, 이전에 그려둔 마스크 선택 영역은 무효화함
...
springai.vision.getMaskBlob = function(canvasId) {
	...
	// 사용자가 별도의 마스크 영역을 지정하지 않았을 경우,
	// 경고 없이 이미지 전체를 편집 대상(전체 투명)으로 처리함
	if (w === 0 && h === 0) {
