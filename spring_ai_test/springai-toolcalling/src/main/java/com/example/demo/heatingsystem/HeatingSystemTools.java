...
public class HeatingSystemTools {
	...
	@Tool(description = """
			...
			""")
	public String startHeatingSystem(@ToolParam(description = "타겟 온도", required = true) int targetTemperature,
			ToolContext toolContext) { // ToolContext를 통해 난방 시스템을 가동할 권한을 확인
		String controlKey = (String) toolContext.getContext().get("controlKey"); // 난방 시스템을 가동할 권한 확인
		if (controlKey != null && controlKey.equals("heatingSystemKey")) { // 난방 시스템을 가동할 권한이 있는 경우
			System.out.println(targetTemperature + " 도까지 난방 시스템을 가동합니다.");
			return "success";
		} else { // 난방 시스템을 가동할 권한이 없는 경우
			System.out.println("난방 시스템을 가동할 권한이 없습니다.");
			return "failure";
		}
	}
	...
	public String stopHeatingSystem(ToolContext toolContext) { // ToolContext를 통해 난방 시스템을 중지할 권한을 확인
		String controlKey = (String) toolContext.getContext().get("controlKey"); // 난방 시스템을 중지할 권한 확인
		if (controlKey != null && controlKey.equals("heatingSystemKey")) {// 난방 시스템을 중지할권한이있는경우
			System.out.println("난방 시스템을 중지합니다.");
			return "success";
		} else {// 난방 시스템을 중지할 권한이 없는 경우
			System.out.println("난방 시스템을 중지할 권한이 없습니다.");
			return "failure";
		}
	}
	...
	public int getTemperature() {
		...
		int temperature = random.nextInt(13) + 18; // 18~30도 사이의 난수 생성
