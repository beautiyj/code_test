...
public class RagService1 {
	...
	public RagService1(ChatClient.Builder chatClientBuilder) {
		this.chatClient = chatClientBuilder.defaultAdvisors(new SimpleLoggerAdvisor(Ordered.LOWEST_PRECEDENCE - 1))
				.build();
	}

	// 4. 벡터 저장소의 데이터를 모두 삭제하는 메소드
	public void clearVectorStore() {
		jdbcTemplate.update("TRUNCATE TABLE rag_vector_store");
	}

	// 5. PDF 파일을 ETL 처리하는 메소드
	public void ragEtl(MultipartFile attach, String source, int chunkSize, int minChunkSizeChars) throws IOException {
		...
		// 변환하기
//		DocumentTransformer transformer = new TokenTextSplitter(chunkSize, minChunkSizeChars, 5, 10000, true);
...
//		vectorStore.add(transformedDocuments);
	...
	public String ragChat(String question, double score, String source) {
		// 벡터 저장소 검색 조건 생성
		SearchRequest.Builder searchRequestBuilder = SearchRequest.builder().similarityThreshold(score).topK(3);
		...
		String answer = this.chatClient.prompt().user(question).advisors(questionAnswerAdvisor).call().content();
		return answer;
	}
