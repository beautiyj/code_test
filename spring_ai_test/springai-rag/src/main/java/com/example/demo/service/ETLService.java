...
public class ETLService {
	...
	public String etlFromFile(String title, String author, MultipartFile attach) throws IOException {
		...
		for (Document doc : documents) {
			...
			metadata.putAll(Map.of("title", title, "author", author, "source", attach.getOriginalFilename()));
		}
...
//		transformedDocuments = keywordMetadataEnricher.apply(transformedDocuments);
		...
		// 메타데이터에 키워드 추가하기 (Gemini 무료 티어 분당 요청 제한 대응: 문서 하나씩 순차 처리 + 딜레이)
	    KeywordMetadataEnricher keywordMetadataEnricher = new KeywordMetadataEnricher(chatModel, 5);
	    List<Document> enrichedDocuments = new java.util.ArrayList<>();

	    for (Document doc : transformedDocuments) {
	        // 문서 1개짜리 리스트로 만들어서 enrich (내부적으로 Gemini Chat 모델 1회 호출)
	        List<Document> enriched = keywordMetadataEnricher.apply(List.of(doc));
	        enrichedDocuments.addAll(enriched);

	        try {
	            Thread.sleep(3500); // 분당 20회 제한 대응 (3.5초 대기 → 분당 약 17회)
	        } catch (InterruptedException e) {
	            Thread.currentThread().interrupt();
	            log.warn("키워드 추출 대기 중 인터럽트 발생", e);
	        }
	    }

	    transformedDocuments = enrichedDocuments;
	...
	public String etlFromHtml(String title, String author, String url) throws Exception {
		...
		JsoupDocumentReader reader = new JsoupDocumentReader(resource,
				JsoupDocumentReaderConfig.builder().charset("UTF-8")
						// 쉼표로 구분해서여러개의셀렉터추가가능
						.selector("#content").metadataTag("author").additionalMetadata(Map.of(
								// "title", title,
								// "author", author,
								"url", url))
						.build());
	...
	public String etlFromJson(String url) throws Exception {
		...
		JsonReader reader = new JsonReader(resource, new JsonMetadataGenerator() {
			...
			public Map<String, Object> generate(Map<String, Object> jsonMap) {
				return Map.of("title", jsonMap.get("title"), "author", jsonMap.get("author"), "url",
						"http://localhost:80/document/constitution(19880225).json");
			}
		}, "date", "content");
