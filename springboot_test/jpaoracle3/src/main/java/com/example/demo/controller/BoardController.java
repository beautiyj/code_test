package springboot_test.jpaoracle3.src.main.java.com.example.demo.controller;
...
public class BoardController {
	...
	public String boardlist(@RequestParam(value = "page", defaultValue = "1") int page,
		...
		// 검색기능 추가
		int listcount = 0;
		// 검색값이 널이 아닌 경우면서 키워드가 널이 아닌 경우 : 검색 데이터 개수
		if (!search.equals("") && !keyword.equals("")) {
			listcount = service.searchCount(search, keyword);
		} else {
			listcount = (int) service.getCount();	// 전체 데이터 개수
//			listcount = (int) service.count();	// 전체 데이터 개수 - 기존의 함수 그대로 써도 무방
		}
		...
		// if 검색 데이터 목록 else 전체 데이터 목록
		if (!search.equals("") && !keyword.equals("")) {
			boardlist = service.searchList(search, keyword);
//			boardlist = service.searchList(start, end, search, keyword); - 기존 함수로 진행할 경우 코드 변경
		} else {
			boardlist = service.getList(start, end);	// 전체 데이터 개수
//			boardlist = service.findAll(page);		// 전체 데이터 개수 - 기존의 함수 그대로 써도 무방
		}
		
		// 총 페이지
//		int pagecount = listcount / limit + ((listcount%limit == 0));
