package springboot_test.jpamysql3.src.main.java.com.example.demo.controller;
...
public class BoardController {
	...
	public String boardlist(@RequestParam(value = "page", defaultValue = "1") int page,
		...
		// 검색기능 추가
		int listcount = 0;
		if (!search.equals("") && !keyword.equals("")) { 	// 검색 데이터 개수
			listcount = service.searchCount(search, keyword);
		} else {											// 전체 데이터 개수
			listcount = (int) service.getCount();
		}

		List<Board> boardlist = null;
		if (!search.equals("") && !keyword.equals("")) { 	// 검색 데이터 목록
			boardlist = service.searchList(start, search, keyword);
		} else {											// 전체 데이터 목록
			boardlist = service.getList(start);
		}
