package springboot_test.thymeleafProjectTest.src.main.java.com.example.demo.controller;
...
//		model.addAttribute("greeting1", null);
		model.addAttribute("greeting1", "greeting1데이터값");
		return "sample1"; // templates/sample1.html
	}
	...
	public String sample4(Model model) {
		List<Member> list = new ArrayList<Member>();
		for (int i = 0; i < 10; i++) {
			Member member = new Member(1, "u000" + i % 3, // 3으로 나눈 나머지 id
	...
	public String sample6(Model model) {
		...
		// Arrays.asList() : 배열을 List 형태로 바로 변환해주는 자바 표준 메소드
	...
	public String sample(@RequestParam("s1") String s1, @RequestParam("s2") String s2, Model model) {
