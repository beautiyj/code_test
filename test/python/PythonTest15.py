...
def is_num(n):  # 만약 n이 505일때
    while n > 0:
        digit = n % 10      # 숫자의 마지막자리 체크하기. 505%10= 나머지 5
        if digit != 0 and digit != 5:   # 마지막 자리가 0,5가 아니면 바로 탈출
            return False
        n //= 10            # 나누기로 자리 제거. 505//10 = 50
    return True

def solution24_t2(l, r):
    temp = [0] * (r - l + 1)
    count = 0
    ...
    """
    실무용 간단버전
    result = [i for i in range(l, r + 1) if all(d in '05' for d in str(i))]
...
def solution24_t3(arr, intervals):
    a1 = intervals[0][0]
    b1 = intervals[0][1]
    a2 = intervals[1][0]
    b2 = intervals[1][1]

    result = [0] * ((b1 - a1 + 1) + (b2 - a2 + 1))
    index = 0
...
if __name__ == "__main__":
    print("Solution24 결과:", solution24(10, 3))
    print("Solution24T2 결과:", solution24_t2(5, 555))

    arr = [1, 2, 3, 4, 5]
    intervals = [[1, 3], [0, 4]]
    print("Solution24T3 결과:", solution24_t3(arr, intervals))